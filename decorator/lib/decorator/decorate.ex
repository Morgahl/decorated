defmodule Decorator.Decorate do
  ...
  defmodule Context do
    ...
    defstruct name: nil, arity: nil, module: nil, args: nil, file: nil, line: nil
  ...
  def on_definition(env, kind, fun, args, guards, body) do
    ...
    decorated = {kind, fun, args, guards, body, decorators, attrs, env}
  ...
  defp decorated_functions(all) do
    Enum.group_by(
      all,
      fn {_kind, fun, args, _guard, _body, _decorators, _attrs, _env} ->
        {fun, Enum.count(args)}
      end,
      fn {_kind, _fun, _args, _guard, _body, decorators, _attrs, _env} ->
        decorators
      end
    )
  ...
  defp filter_undecorated(all, decorated_functions) do
    all
    |> Enum.filter(fn {_kind, fun, args, _guard, _body, _decorators, _attrs, _env} ->
      Map.has_key?(decorated_functions, {fun, Enum.count(args)})
    end)
  end

  defp reject_empty_clauses(all) do
    Enum.reject(all, fn {_kind, _fun, _args, _guards, body, _decorators, _attrs, _env} ->
      body == nil
    end)
  end
  ...
  defp decorate(
         env,
         {kind, fun, args, guard, body, decorators, attrs, on_def_env},
         decorated_functions,
         {prev_funs, all}
       ) do
    override_clause =
    ...
    arity = Enum.count(args || [])
    context = %Context{name: fun, arity: arity, args: args, module: env.module, file: on_def_env.file, line: on_def_env.line}
